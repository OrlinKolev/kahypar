 ...
 public:
  ...
  void activate(const HypernodeID hn,
    ...
    if (_hg.isBorderNode(hn)) {
      ...
      DBG(dbg_refinement_2way_netstatus__activation, "inserting HN " << hn << " with gain "
 ...
 private:
  ...
  bool refineImpl(std::vector<HypernodeID>& refinement_nodes,
    ...
    while (!_pq.empty() &&
      ...
      if (move_is_feasible) {
        DBG(dbg_refinement_2way_netstatus_improvements_balance && max_gain == 0,
            "2WayFM improved balance between " << from_part << " and " << to_part
            << "(max_gain=" << max_gain << ")");
        DBG(dbg_refinement_2way_netstatus_improvements_cut && current_cut < best_metrics.cut,
    ...
    DBG(dbg_refinement_2way_netstatus_stopping_crit, "KWayFM performed " << _performed_moves.size()
  ...
  void updatePin(const HypernodeID pin, const Gain gain_delta) KAHYPAR_ATTRIBUTE_ALWAYS_INLINE {
    ...
    DBG(dbg_refinement_2way_netstatus_gain_update, "TwoWayFM updating gain of HN " << pin
