...
class TwoWayThThSoftGainRefiner final : public IRefiner,
 ...
 public:
  TwoWayThThSoftGainRefiner(Hypergraph& hypergraph, const Context& context) :
  ...
  ~TwoWayThThSoftGainRefiner() override = default;

  TwoWayThThSoftGainRefiner(const TwoWayThThSoftGainRefiner&) = delete;
  TwoWayThThSoftGainRefiner& operator= (const TwoWayThThSoftGainRefiner&) = delete;

  TwoWayThThSoftGainRefiner(TwoWayThThSoftGainRefiner&&) = delete;
  TwoWayThThSoftGainRefiner& operator= (TwoWayThThSoftGainRefiner&&) = delete;
 ...
 private:
  ...
  void updatePin(const HypernodeID pin, const FineGain gain_delta) KAHYPAR_ATTRIBUTE_ALWAYS_INLINE {
    ...
    DBG << "TwoWayThThSoftGain updating gain of HN" << pin
  ...
  FineGain getThresholdFactor(HypernodeID t, HypernodeID sz) const {
    // the hard gain cases
    if (t == 0 || t == sz) {
      // edge is commpletely in the one or the other part; any move would add it to the cut
      return -1;
    } else if (t == sz - 1) {
      // one node remaining on this part
      return 1;
    }

    FineGain l_part = (FineGain)(t-1) / sz;
    FineGain m_part = (FineGain)t / sz;
    FineGain r_part = (FineGain)(t+1) / sz;

    auto l = std::lower_bound(_thresholds.begin(), _thresholds.end(), l_part);
    auto m = std::lower_bound(_thresholds.begin(), _thresholds.end(), m_part);
    auto r = std::lower_bound(_thresholds.begin(), _thresholds.end(), r_part);

    int li = l - _thresholds.begin();
    int mi = m - _thresholds.begin();
    int ri = r - _thresholds.begin();

    FineGain sum = 0;
    for (int i = mi; i < ri; i++) {
      ASSERT(i >= 0);
      ASSERT(i < _th_gain_factors.size());
      sum += _th_gain_factors[i];
    }
  ...
  std::vector<double> _th_gain_factors;
};
}                                   // namespace kahypar
